...
export function initCalendar() {
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        datesSet(info) {
            ...
            // "Today" is only useful when you have navigated away from the
            // current month, so it stays hidden otherwise. The off-month
            // class lets narrow screens make room for the chip.
            const now = new Date();
            const viewStart = info.view.currentStart;
            const onCurrentMonth =
                viewStart.getFullYear() === now.getFullYear() &&
                viewStart.getMonth() === now.getMonth();
            if (todayBtn) todayBtn.hidden = onCurrentMonth;
            monthLabel.closest('.month-nav')
                ?.classList.toggle('off-month', !onCurrentMonth);
        },
